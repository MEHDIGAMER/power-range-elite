...
Opus:   CTO, Project Manager, Architect, Tech Lead, What-If Agent, Shock Wave
Sonnet: Backend, Frontend, Challenger, Integration, Role & Access,
        QA, Code Reviewer, Business KPI
...
## PHASE 3.5 — CODEBASE INTELLIGENCE MAP

After generating all project files, run a full codebase scan using /power-mapout.

Tell user: "Building codebase intelligence map — this scans every function, maps dependencies, and scores blast radius + health zones..."

Execute /power-mapout in FULL SCAN mode:
1. Scan all project files using the mapper subagents described in /power-mapout Phase 2
2. Build the dependency graph (Phase 3)
3. Calculate blast radius + health zone scores (Phase 4)
4. Write output files (Phase 5):
   - .power-mapout/graph.json
   - .power-mapout/CODEMAP.md
   - .power-mapout/briefings/

After the map is generated, enrich the project files created in Phase 3:

**Update BOOKKEEPER.md:**
- Replace "Dependency Chains — DANGER ZONES" section with actual danger zones from .power-mapout/CODEMAP.md
- Add shared resource map (connection pools, caches, global state) to "External Integration Points"
- Fill the Component Registry with accurate dependency data from the graph

**Update .power-range/config.md:**
- Replace "High-Risk Files" with the top 5 CRITICAL blast radius nodes from the map
- Replace "Known Fragile Areas" with RED zone nodes from the map

Tell user:
```
Codebase intelligence map built:
  Nodes: [N] functions mapped
  Edges: [N] dependencies traced
  CRITICAL nodes: [N] (blast radius >= 8)
  RED zone: [N] (health <= 0.30)
  Map: .power-mapout/CODEMAP.md
```
...
MISTAKES.md
.power-range/config.md
.power-mapout/graph.json        (machine-queryable dependency graph)
.power-mapout/CODEMAP.md        (human-readable intelligence map)
...
Codebase intelligence map is active — agents will use it automatically.
